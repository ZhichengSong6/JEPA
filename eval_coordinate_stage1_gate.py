#!/usr/bin/env python3
"""Life-or-death gate for Coordinate Stage-I on exact official CEM populations.

Scientific question
-------------------
Does the learned global coordinate transform
...
only to recover endpoint images, and scores every candidate with:
    ...
    2) Coordinate-Stage-I raw encoder/predictor cost.

The exported Coordinate model conjugates the frozen predictor exactly,
...
def _base_weights_digest(model) -> str:
    """Hash all pretrained weights while excluding the Stage-I adapter."""
    h = hashlib.sha256()
...
def run(cfg: DictConfig):
    ...
    coordinate_policy = str(
        dc.get(
            "coordinate_policy",
            "pusht_coordinate_stage1_seed3072/lewm_coordinate_stage1",
        )
    )
    primary_cases = [
    ...
    if adapter is None:
        raise RuntimeError("Coordinate policy has no coordinate_adapter.")

    base_digest = _base_weights_digest(base)
    coord_digest = _base_weights_digest(coordinate)
    if base_digest != coord_digest:
        raise RuntimeError(
            "Coordinate Stage-I changed pretrained model weights. "
    ...
    summary = {
        "status": "complete",
        "scientific_question": (
            "Can one global invertible latent coordinate transform improve "
        ...
        "interpretation_rule": (
            "Stage-I is supported only if hard-case raw ordering improves "
            "meaningfully while matched-success controls are not materially "
            "degraded. Improved local controllability condition alone is not "
            "sufficient."
        ),
    }
    write_json(out / "summary.json", summary)

    print("\n===== COORDINATE STAGE-I LIFE/DEATH GATE =====")
