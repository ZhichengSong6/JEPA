"""Train Planner-Context Goal-Sensitive Multi-Horizon ALD on PushT.

Controlled continuation of formal MH-ALD:
...
* objective          : original TF + planner-context rollout + PC-GP-MH
...
from planner_context_goal_mh import pc_gp_mh_ald_forward
...
@hydra.main(
    ...
    config_name="lewm_pc_gp_mh_ald",
)
def run(cfg):
    pl.seed_everything(int(cfg.seed), workers=True)

    if not bool(cfg.mh_ald.enabled):
        raise ValueError("PC-GP-MH requires mh_ald.enabled=True.")
    if not bool(cfg.mh_ald.planner_context.enabled):
        raise ValueError("PC-GP-MH requires planner_context.enabled=True.")
    if not bool(cfg.mh_ald.goal_sensitive.enabled):
        raise ValueError("PC-GP-MH requires goal_sensitive.enabled=True.")
    if "state" in cfg.data.dataset.keys_to_load:
        raise ValueError("PC-GP-MH must not load privileged simulator state.")

    expected_steps = int(cfg.wm.history_size) + int(cfg.mh_ald.rollout_horizon)
    if int(cfg.data.dataset.num_steps) != expected_steps:
        raise ValueError(
            "PC-GP-MH sequence length mismatch: expected "
    ...
    if float(cfg.mh_ald.goal_sensitive.parallel_multiplier) < 1.0:
        raise ValueError("parallel_multiplier must be >= 1.")
    ...
    print(f"Loading PC-GP-MH student init: {cfg.mh_ald.init_policy}")
    student = _load_policy_model(cfg.mh_ald.init_policy)
    student.requires_grad_(True)
    _freeze_visual_latent_frame(student)
    _train_predictor_side(student)

    print(f"Loading frozen PC-GP-MH teacher: {cfg.mh_ald.teacher_policy}")
    ...
    if visual_trainable != 0:
        raise RuntimeError(
            f"PC-GP-MH visual latent frame is not frozen: {visual_trainable}."
        )

    print(
        "PC-GP-MH parameter summary: "
    ...
    print(
        ...
        f"parallel_multiplier={float(cfg.mh_ald.goal_sensitive.parallel_multiplier):.2f} "
