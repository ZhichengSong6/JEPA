"""Train Anchored Local Dynamics (ALD) calibration on full PushT.
...
* objective          : one-step TF + H-step rollout + ALD
...
from anchored_local_dynamics import ald_forward
...
@hydra.main(
    ...
    config_name="lewm_ald",
)
def run(cfg):
    pl.seed_everything(int(cfg.seed), workers=True)

    if not bool(cfg.ald.enabled):
        raise ValueError("train_ald.py requires ald.enabled=True.")
    if "state" in cfg.data.dataset.keys_to_load:
        raise ValueError(
            "ALD must not load privileged simulator state. Use data=pusht_ald."
        )

    expected_steps = int(cfg.wm.history_size) + int(cfg.ald.rollout_horizon)
    if int(cfg.data.dataset.num_steps) != expected_steps:
        raise ValueError(
            "ALD sequence length mismatch. Expected "
            f"wm.history_size + ald.rollout_horizon = {expected_steps}, "
    ...
    # Student / teacher.
    # ------------------------------------------------------------------
    print(f"Loading ALD student init: {cfg.ald.init_policy}")
    student = _load_policy_model(cfg.ald.init_policy)
    student.requires_grad_(True)
    _freeze_visual_latent_frame(student)
    _train_predictor_side(student)

    print(f"Loading frozen ALD teacher: {cfg.ald.teacher_policy}")
    teacher = _load_policy_model(cfg.ald.teacher_policy)
    teacher.eval()
    ...
    if visual_trainable != 0:
        raise RuntimeError(
            f"ALD visual latent frame is not frozen: {visual_trainable} trainable params."
        )

    print(
        "ALD parameter summary: "
    ...
    training_module = spt.Module(
        model=student,
        # SIGReg has buffers only and is not used in ald_forward.  Keeping the
