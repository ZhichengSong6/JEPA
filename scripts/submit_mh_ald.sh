#!/usr/bin/env bash
# Submit Multi-Horizon Anchored Local Dynamics (MH-ALD) calibration on all four GPUs of a chosen 4090 node.
#
# Usage:
#   bash scripts/submit_mh_ald.sh smoke
#   bash scripts/submit_mh_ald.sh formal
#   NODE=4090node3 bash scripts/submit_mh_ald.sh formal
#
...
# Its initial validation should satisfy the MH-ALD construction invariants:
#   mh_ald_response_cosine ~= 1
#   mh_ald_response_gain   ~= 1
#   mh_ald_half_response_loss ~= 0
#   mh_ald_init_equivalence_ratio ~= 1

set -euo pipefail

MODE="${1:-}"
if [[ "$MODE" != "smoke" && "$MODE" != "formal" ]]; then
  echo "Usage: bash scripts/submit_mh_ald.sh {smoke|formal}" >&2
  exit 2
fi
...
if [[ "$NODE" != "4090node2" && "$NODE" != "4090node3" ]]; then
  echo "ERROR: MH-ALD smoke/formal jobs are restricted to 4090node2 or 4090node3." >&2
  exit 4
fi
WORLD_SIZE=4
LOCAL_BATCH=16
...
GEN_DIR="$REPO/slurm/generated_mh_ald_ddp4"
LOG_DIR="$REPO/logs"
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  cat > "$FILE" <<EOF
#!/bin/bash
#SBATCH --job-name=mhald_sm4
$COMMON_HEADER
#SBATCH --output=$LOG_DIR/mh_ald_smoke_%j.out
#SBATCH --error=$LOG_DIR/mh_ald_smoke_%j.err
...
$COMMON_HEADER
#SBATCH --output=$LOG_DIR/mh_ald_formal_%j.out
#SBATCH --error=$LOG_DIR/mh_ald_formal_%j.err
...
echo "Submitted MH-ALD ($MODE): $JID"
echo "node         : $NODE"
...
echo
if [[ "$MODE" == "smoke" ]]; then
  echo "Log: tail -f $LOG_DIR/mh_ald_smoke_${JID}.out"
  echo "After the smoke completes cleanly, submit:"
  echo "  NODE=$NODE bash scripts/submit_mh_ald.sh formal"
else
  echo "Log: tail -f $LOG_DIR/mh_ald_formal_${JID}.out"
  echo "Final model:"
